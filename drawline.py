...
def __repr__(self):
return 'ViewPort(%s, %s, %s, %s)' %
(self.xVmin,self.yVmin,self.xVmax,self.yVmax)
def init_view(self):
...
(self.xwmin,self.ywmin,self.xwmax,self.ywmax)
def map_to(self,x_win,y_win,ViewPort):

x_view = (x_win-self.xwmin)*(ViewPort.xVmax -
ViewPort.xVmin)/(self.xwmax-self.xwmin) + ViewPort.xVmin
y_view = (y_win-self.ywmin)*(ViewPort.yVmax -
ViewPort.yVmin)/(self.ywmax-self.ywmin) + ViewPort.yVmin
...
if x2==x1==-1:
x1,y1,x2,y2 = map(int,input('Enter Line\'s Endpoints <x1> <y1> <x2>
<y2> :').split())
...
def main(win=None):
if len(sys.argv[1:])!=4:
exit(print('Wrong Number of Arguments','\nUsage :python3 drawLine.py <x1>
<y1> <x2> <y2>'))
if(input('Defualt viewPort is (-400 -400, 400 400). Change?(y/Y)')in
('y','Y')):
x,y=map(int,input('viewPort\'s xMax yMax : ').split())
new_view = ViewPort(-x,-y,x,y)
else:
new_view =ViewPort(-400,-400,400,400)
if(input('Defualt Window is (-200 -200, 200 200). Change?(y/Y)')in
('y','Y')):
